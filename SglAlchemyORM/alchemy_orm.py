...
class Author(Base):  # klasa o naszych Użytkownikach, którzy mają odpowiedniki w tabeli
    ...
    id: Mapped[
        intpk]  # było = mapped_column(primary_key=True, autoincrement=True) ale dajemy intpk #definiujemy zmienna na naszej Klasie i kolumne INTEGER, Klucz Glowny i autoinkrementalna
    name: Mapped[str] = mapped_column(String())  # odpowiednik VARCHAR(MAX)
    email: Mapped[
        str255]  # było = mapped_column(String(255)) ale dzieki Annotated nie musze tego pisac #string o rozmiarze 255 bite'ow
    login: Mapped[str] = mapped_column(String(100), default='No Login')
    middle_name: Mapped[Optional[
        str]]  # było tak = mapped_column(String(255), nullable=True) ale dzięki optional (import na gorze) możemy usunąć

    books: Mapped[List['Book']] = relationship(back_populates='author',
                                               cascade='all, delete, delete-orphan')  # połączenie z relationship(back_populates='books')
...
class Book(Base):
    ...
    author: Mapped['Author'] = relationship(
        back_populates='books')  # połączenie z relationship(back_populates='author')
